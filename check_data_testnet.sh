...
DATA_DIR=".rosetta-cli-data-testnet"
...
if [ $BOOTSTRAP_BALANCES ]; then
  ...
  docker run -v /etc/ssl/certs:/etc/ssl/certs:ro -it --rm --entrypoint /rosetta-iota-utils rosetta-iota_rosetta --network testnet7 --bech32-hrp atoi
  ROSETTA_UTILS_EXIT=$?
